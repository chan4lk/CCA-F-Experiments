...
Write `gaps.md` in exactly this shape. The orchestrator parses the `## G<n> —` headings to
decide whether to run another research round and what to send it, so a malformed heading
loses the gap.
