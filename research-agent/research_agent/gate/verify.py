...
def _run_economics(workspace: Path) -> dict | None:
    """What the agents cost, from run-log.jsonl.

    The plugin measured the *orchestrator's* context growth, because the
    orchestrator was a model and its own prose was 65% of the bill. This
    orchestrator is a Python function with no context to grow, so the number
    worth reporting moved: it is now what the agents cost, which is the whole
    bill. Reported here so it lands next to the work it bought.
    """
    rows = read_jsonl(Path(workspace) / "run-log.jsonl")
    if not rows:
        return None
    ...
    for row in rows:
        ...
        bucket = by_role.setdefault(role, {"dispatches": 0, "cost_usd": 0.0, "turns": 0})
        bucket["dispatches"] += 1
        bucket["cost_usd"] += float(row.get("cost_usd") or 0.0)
        bucket["turns"] += int(row.get("num_turns") or 0)
...
def collect_stats(ctx: Context) -> dict:
    ...
    return {
        ...
        "runs": _run_economics(ctx.workspace),
...
def render_report(findings: list[Finding], stats: dict, passed: bool) -> str:
    ...
    if runs:
        lines += [
            "", "## Run economics", "",
            f"- Agent dispatches: {runs['dispatches']:,}"
            + (f" ({runs['errors']} errored)" if runs["errors"] else ""),
            f"- Total agent cost: ${runs['cost_usd']:.4f}",
            "",
            "| Role | Dispatches | Turns | Cost |",
            "|---|---|---|---|",
        ]
        lines += [
            f"| {role} | {b['dispatches']} | {b['turns']} | ${b['cost_usd']:.4f} |"
            for role, b in sorted(runs["by_role"].items(),
