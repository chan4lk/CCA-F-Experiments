...
import research_agent_batch_server_tools.ledger.claims as add_claim
...
def test_missing_fetch_log_names_the_likely_cause(tmp_path, capsys):
    """An empty log means nothing has been retrieved yet.

    In the plugin this pointed at research/.active.json, the session-id map whose
    mis-registration silently emptied the log for a whole run. This port has no
    such file — the workspace is closed over by the hook — so the hint is now the
    only remaining cause: WebFetch the page before claiming from it.
    ...
    assert rc == 0
    assert "PROVENANCE" in err
    assert "Call WebFetch on the URL" in err
