...
def test_a_run_round_trips_through_the_file(tmp_path):
    ...
    run.tasks = [Task(
        ...
        # The domain pin lives on the tool definition now, not on the task, so
        # the thing that has to survive a resume is the whole grant.
    ...
    assert loaded.tasks[0].tools[0]["allowed_domains"] == ["learn.microsoft.com"]
...
def test_retiring_banks_what_the_phase_spent(tmp_path):
    ...
    run = a_state(tmp_path)
    conv = Task(custom_id="a", role="planner", model="claude-sonnet-5",
                system="s", messages=[])
...
def test_active_done_and_failed_partition_the_wave(tmp_path):
    ...
    for index, status in enumerate(statuses):
        conv = Task(custom_id=f"c{index}", role="validator",
                            model="claude-haiku-4-5", system="s", messages=[])
        conv.status = status
        run.tasks.append(conv)
...
def test_cost_counts_the_wave_in_flight_as_well_as_retired_ones(tmp_path):
    run = a_state(tmp_path)
    first = Task(custom_id="a", role="planner", model="claude-sonnet-5",
                         system="s", messages=[])
    first.input_tokens = 1_000_000
    run.tasks = [first]
    run.retire()

    second = Task(custom_id="b", role="planner", model="claude-sonnet-5",
                          system="s", messages=[])
...
def test_a_retired_phase_banks_what_it_spent_on_searching(tmp_path):
    """Searching is a line item of its own here. Banking only the tokens would
    lose it at every phase boundary, and researching is where the searches are."""
    run = a_state(tmp_path)
    conv = Task(custom_id="a", role="researcher", model="claude-sonnet-5",
                system="s", messages=[])
    conv.web_searches = 100
    run.tasks = [conv]
    run.retire()
    run.save()
    assert st.RunState.load(tmp_path).cost_usd == pytest.approx(1.0)
