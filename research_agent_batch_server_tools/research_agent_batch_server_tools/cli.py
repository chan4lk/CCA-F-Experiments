...
def normalize_argv(argv: list[str]) -> list[str]:
    """`research_agent_batch_server_tools "a question"` should not have to say `research`."""
    argv = list(argv)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="research_agent_batch_server_tools",
        description="Cited proposal research on the Message Batches API with "
                    "server-side tools: half price, one batch per phase, resumable "
                    "across days.")
    ...
    cont = sub.add_parser("resume", help="advance a run: collect the batch, submit the next phase")
...
def summarise(run: st.RunState) -> None:
    ...
    if run.phase == st.AWAITING_APPROVAL:
        ...
        print(f"When you have approved it:  python -m research_agent_batch_server_tools draft "
        ...
        print(f"Check in with:  python -m research_agent_batch_server_tools status "
              f"--workspace {workspace}")
        print(f"Continue with:  python -m research_agent_batch_server_tools resume "
              f"--workspace {workspace}")
