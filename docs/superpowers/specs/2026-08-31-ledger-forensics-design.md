...
| A genuine identifier inside a token of 20+ characters drawn entirely from `[0-9a-f-]` is classified as a digest and missed | Such a token is indistinguishable from a fingerprint. The alternative — no digest rule — halted real audit runs on every GUID and container ID in tool output, measured at 1 in 171 digests. Three successive attempts at a bounded window each created a worse hole; the total rule is the one that converged |
| A database tool using both an unconventional argument name and a single-object statement with no clause word (`DELETE anon_table` under a key named `payload`) is not caught by L4 | Treating "keyword plus one identifier" as SQL-shaped would block an ordinary note like "delete row", and one over-blocking fix has already had to be undone. L1's read-only grant plus `db_denydatawriter` is the actual control for writes (D2); this hook is defence in depth |
