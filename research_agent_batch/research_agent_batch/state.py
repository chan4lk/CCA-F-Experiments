...
class RunState:
    ...
    subject: str = ""
    # What finished phases spent, banked when their conversations are cleared.
    ...
    def active(self) -> list[Conversation]:
        return [c for c in self.conversations if c.active]

    @property
    def done(self) -> list[Conversation]:
        return [c for c in self.conversations if c.status == "done"]

    @property
    def failed(self) -> list[Conversation]:
        return [c for c in self.conversations if c.status == "failed"]
    ...
    def cost_usd(self) -> float:
        ...
        """
        return (sum(c.cost_usd for c in self.conversations)
    ...
    def retire(self, extra: list[dict[str, Any]] | None = None) -> None:
        ...
        a full transcript of every agent that has ever run — but the money they
        spent has to survive, or the run's total silently resets each phase.
        """
        self.retired += [
            {"role": c.role, "model": c.model, "cost_usd": c.cost_usd,
             "input_tokens": c.input_tokens, "output_tokens": c.output_tokens,
             "rounds": c.round, "status": c.status}
