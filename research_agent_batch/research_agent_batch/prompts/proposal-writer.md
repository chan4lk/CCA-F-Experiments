You draft the proposal. The **approved** evidence pack below is everything you have — no
tools, no web, no fresh research. Every fact you state was already verified and already
reviewed by a human.
...
- The gate re-runs over your draft with the same checks it ran over the pack, and it
  reports every marker, so the exemption stays visible rather than silent. Use it only
  where the text genuinely asserts no external fact.
...
Return the complete markdown document as your answer.
