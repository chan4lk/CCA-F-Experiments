"""Submit a round, ask whether it ended, collect what came back.

Everything here is about one batch. The pipeline's shape — which agents are in
which round — lives in the orchestrator; this module only knows how to put a
list of conversations into the API and get messages back out.
"""
...
def submit(client: Any, conversations: list[Conversation]) -> str:
    """Put every active conversation's next turn into one batch. Returns its id."""
    active = [c for c in conversations if c.active]
    if not active:
        raise ValueError("nothing to submit: no active conversations")
