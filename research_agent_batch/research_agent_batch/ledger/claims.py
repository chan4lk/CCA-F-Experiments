...
def provenance_warning(workspace: Path, url: str) -> str:
    ...
    if not log.is_file() or not log.stat().st_size:
        return (
            ...
            f"  Call WebFetch on the URL before recording a claim for it."
        )
    ...
    return (
        ...
        f"  Call WebFetch on the same URL now — the claim has been appended, and the "
        f"retrieval will attach to it retroactively."
    )


def append_claim(workspace: Path, row: dict) -> tuple[bool, str, str]:
    """Validate and atomically append one claim. Returns ``(ok, message, warning)``.

    The single path a claim takes into the ledger. Both the ``add_claim`` MCP
    tool the researcher calls and the CLI below go through here, so validation
    cannot differ between them.
    """
    ledger = Path(workspace) / "claims.jsonl"
    existing_ids = {r["id"] for r in read_jsonl(ledger) if r.get("id")}

    errors = validate_claim(row, existing_ids)
    if errors:
        return False, "claim not appended. Fix and retry:\n" + "\n".join(
            f"  - {e}" for e in errors), ""

    row.setdefault("fetched_at", utc_now())
    append_jsonl(ledger, row)
    return True, f"appended {row['id']}", provenance_warning(Path(workspace), row["url"])
...
def main(argv: list[str] | None = None) -> int:
    ...
    ok, message, warning = append_claim(Path(args.workspace), row)
    if not ok:
