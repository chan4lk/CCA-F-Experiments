"""Submitting a round and collecting what comes back."""
from fakes import FakeClient, text_message

from research_agent_batch.batching import (
    Failure,
    collect,
    counts,
    has_ended,
...
def conversation(custom_id="a", status="active") -> Conversation:
    conv = Conversation(custom_id=custom_id, role="validator", model="claude-haiku-4-5",
...
def test_one_batch_carries_every_active_conversation():
    client = FakeClient(lambda *a: text_message("{}"))
    batch_id = submit(client, [conversation("a"), conversation("b")])
    assert batch_id.startswith("msgbatch_")
    assert [r["custom_id"] for r in client.batches.submitted[0]] == ["a", "b"]


def test_a_finished_conversation_is_not_resubmitted():
    """Resubmitting a done agent is a request that is paid for and discarded."""
    client = FakeClient(lambda *a: text_message("{}"))
    submit(client, [conversation("a"), conversation("b", status="done")])
...
def test_submitting_nothing_is_a_bug_not_an_empty_batch():
    client = FakeClient(lambda *a: text_message("{}"))
    try:
        submit(client, [conversation("a", status="done")])
    except ValueError as exc:
        assert "no active conversations" in str(exc)
    else:
        raise AssertionError("expected a ValueError")


def test_each_request_carries_its_conversations_params():
    client = FakeClient(lambda *a: text_message("{}"))
    submit(client, [conversation("a")])
...
def test_successes_are_keyed_by_custom_id():
    ...
    submit(client, [conversation("a"), conversation("b")])
